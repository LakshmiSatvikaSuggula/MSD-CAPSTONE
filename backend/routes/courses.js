...
const storage = new CloudinaryStorage({
  ...
  params: async (req, file) => {
    const field = file.fieldname;
    let folder = 'courses', resource_type = 'auto';
    if (field === 'video') {
      folder = 'courses/videos'; resource_type = 'video';
    } else if (field === 'pdf' || field === 'ppt') {
      folder = 'courses/files'; resource_type = 'raw';
    } else if (field === 'thumbnail') {
...
// ✅ POST new course (uses hostEmail)
router.post('/', parser.fields([
  { name: 'video', maxCount: 5 },
  { name: 'pdf', maxCount: 5 },
  { name: 'ppt', maxCount: 5 },
  ...
  try {
    const { title, description, drive, hostEmail } = req.body;
    if (!title || !hostEmail) return res.status(400).json({ error: 'Title and hostEmail required' });

    const videos = (req.files['video'] || []).map(f => ({
      url: f.path, public_id: f.filename || f.public_id, resource_type: 'video'
    }));
    const pdfs = (req.files['pdf'] || []).map(f => ({
      url: f.path, public_id: f.filename || f.public_id, resource_type: 'raw'
    }));
    const ppts = (req.files['ppt'] || []).map(f => ({
      url: f.path, public_id: f.filename || f.public_id, resource_type: 'raw'
    }));
    ...
    const thumbnail = thumbnailFile ? {
      ...
      public_id: thumbnailFile.filename || thumbnailFile.public_id,
      resource_type: 'image'
    } : undefined;

    const course = new Course({
      title, description, drive, hostEmail,
      videos, pdfs, ppts, thumbnail
    });
